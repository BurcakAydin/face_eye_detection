...
def detect_faces_and_eyes(image_pil):
    ...
    image = np.array(image_pil.convert('RGB'))  # Ensure the image is in RGB
    gray = cv.cvtColor(image, cv.COLOR_RGB2GRAY)  # Convert image to grayscale for face detection
    ...
    # Convert the processed NumPy image array back to a PIL image
    result_image_pil = Image.fromarray(image)
    return result_image_pil  # Return as a PIL image for compatibility with Streamlit
...
if uploaded_file is not None:
    ...
    if st.button('Detect Faces and Eyes'):
        # Process the image to detect faces and eyes
